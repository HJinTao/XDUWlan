...
学习者按 Python 和计算机网络初学者对待。每个网络小步先说明当前功能在完整通信路径中的位置，解释通信参与方、所在层次、输入输出、正常数据流和当前相关的失败现象；随后把网络概念映射到具体 API、项目符号和测试，再进入实现。概念分层、操作系统或网络库接口、本项目抽象必须分别说明，网络知识只随当前切片需要渐进展开。
