...
所有文档使用中文。每完成一个可验证阶段或当前状态发生实质变化时，更新 `docs/progress.md` 和 `docs/session-handoff.md`；不因单纯对话机械扩写。新会话先读 `AGENTS.md`、交接文档、进度文档，再读当前学习和协议文档。Git 提交记录代码变化，决策文档记录原因，进度文档记录当前位置。任务完成时按职责分批提交，并在不覆盖学习者修改的前提下检查工作区是否干净；详细协作规则以 `AGENTS.md` 为准。

### 10.1 教学实施顺序

实现采用纵向切片，以尽早形成学习者可以运行和观察的完整命令。顺序为 `status`、`configure`、`login`、`watch`、`account`。每个切片按“解释当前语法和网络概念 → 指导者写失败测试与骨架 → 学习者实现边界清晰的连贯逻辑 → 指导者审查并运行测试 → 接入 CLI → 实际运行 → 更新中文文档”的循环推进。

学习者按 Python 和计算机网络初学者对待。每个网络小步先说明当前功能在完整通信路径中的位置，解释通信参与方、所在层次、输入输出、正常数据流和当前相关的失败现象；随后把网络概念映射到具体 API、项目符号和测试，再进入实现。概念分层、操作系统或网络库接口、本项目抽象必须分别说明，网络知识只随当前切片需要渐进展开。实现单元按连贯概念、函数或紧密相关状态划分，可随理解程度调整大小，但不得机械拆分同一逻辑，且每轮保留清晰的 RED/GREEN 信号。学习者负责业务逻辑，指导者可以在不改变行为的前提下完成格式和描述整理。
