...
- 学习者完成业务逻辑后，指导者可以处理不改变行为的格式与文档描述整理；可能改变控制流、数据或外部行为的修改不属于格式整理。
- 实施采用纵向切片，顺序为 `status`、`configure`、`login`、`watch`、`account`；每个切片穿插所需模型、配置、服务、适配器、CLI、测试与文档，不提前完成未来切片的底层模块。
- 学习者按 Python 初学者对待；每一步先解释本步实际出现的语法，再由学习者实现一个边界清晰、逻辑连贯的单元。同一函数或同一概念内的相关分支可以合并推进，不按固定时长机械拆分，但每轮必须保留清晰的 RED/GREEN 信号。
...
- 每个任务完成后运行该任务的测试，更新 `docs/progress.md` 和 `docs/session-handoff.md`；有多类改动时按职责分批提交，逐批检查暂存差异，最后验证测试并确认 `git status --porcelain` 为空。学习者修改或来源不明的改动不得为清理工作区而擅自纳入提交。
...
每个切片都重复“语法与原理讲解 → 指导者写当前失败测试和骨架 → 学习者实现边界清晰的连贯逻辑 → 指导者审查与 GREEN 验证 → CLI 运行 → 文档与按职责提交”。
